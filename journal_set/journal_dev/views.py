    ...
    for k, v in us_journal.items():
        for value in v:
            if value in lf_name:
                nm = k
    print(nm)
    obj = journal.objects.filter(nomenklatura__icontains = nm).order_by('-id')
    print(obj.count())
    obj1 = journal.objects.filter(nomenklatura__icontains = nm).exclude(signature__nomenklatura__icontains = nm)
    print(obj1.count())
